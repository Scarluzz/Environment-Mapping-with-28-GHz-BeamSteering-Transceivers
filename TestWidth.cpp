...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    //int Rx_fixed_steer = -40;

    //OutFilename = "AntennaPattern_" + std::to_string(nbr_active_antennas) + "_" + std::to_string(Rx_fixed_steer) +  "_.dat";

    OutFilename = "AntennaPattern_" + std::to_string(nbr_active_antennas) + ".dat";


    /*THESE VALUES ARE HARDCODED AND SHOULD BE REDEFINED WHENEVER THE DESIGNED SIGNAL IS DIFFERENT !*/
    double          Nr_Of_Tx_beams      = 19;
    double          Nr_Of_Rx_beams      = 1;
	...
	("gain-tx-bb", po::value<double>(&gain_tx_bb)->default_value(15), "Gain of Tx baseband signal in dB")
	("gain-rx-bb", po::value<double>(&gain_rx_bb)->default_value(37.5), "Gain of Rx baseband signal in dB")
    ...
    //("OutFile", po::value<std::string>(&OutFilename)->default_value("AntennaPattern.dat"), "name of the file to read binary samples from")
    ("InputFile", po::value<std::string>(&InFilename)->default_value("QPSK.dat"), "name of the output file")   
    ...
    ("theta_min_tx", po::value<int>(&theta_min_tx)->default_value(0), "Min TX Theta (degree)")
    ("theta_max_tx", po::value<int>(&theta_max_tx)->default_value(0), "Max TX Theta (degree)")
    ...
    //("nantennas", po::value<int>(&nbr_active_antennas)->default_value(4), "Number of active antennas")
    ;
    ...
    std::cout << "Default value of Rx sampling rate is " << rx_rate << std::endl;
    std::cout << "Oversampling is " << OverSampling << std::endl;
    std::cout << "Upsample is " << UpsampleFactor << std::endl;
    ...
    if (nbr_active_antennas == 2) { 
        int temp[] = {3, 4}; // Keeping only 2 channels
    ...
    else if (nbr_active_antennas == 3){
        int temp[] = {4}; // Turning off only one channel
        nbr_channels = sizeof(temp) / sizeof(temp[0]);
        memcpy(off_channels, temp, sizeof(temp));
    }
    else if (nbr_active_antennas == 0){
        int temp[] = {1, 2, 3, 4};
        nbr_channels = sizeof(temp) / sizeof(temp[0]);
        memcpy(off_channels, temp, sizeof(temp));
    }
    ...
    std::cout << "\n\nRequested (BBox Antennas!) gains (RX/TX) using dynamic range is : " << gain_dB_Rx << ", " << gain_dB_Tx << std::endl << std::endl;

    //double thetahere = Rx_fixed_steer;
    set_theta(service, sn_Rx_BBox, gain_dB_Rx, 0);

    //thetahere = 0;
    set_theta(service, sn_Tx_BBox, gain_dB_Tx, 0);
    ...
    for (int i = 0; i < 4; i++)
    {
        std::cout << i << "th element of OFF_CHANNELS is : " << (int)off_channels[i] << std::endl;
    }

    std::cout << "Turning off " << nbr_channels << " channels." << std::endl;
    ...
    uhd::gain_range_t Tx_usrp_gain_range = radio_ctrl->get_tx_gain_range(radio_chan);
    uhd::gain_range_t Rx_usrp_gain_range = usrp_rx_bb->get_rx_gain_range();

    std::cout << "\n\nGain range at TX usrp (radio block) : " << Tx_usrp_gain_range.to_pp_string() << std::endl << std::endl;
    std::cout << "\n\nGain range at RX usrp (radio block) : " << Rx_usrp_gain_range.to_pp_string() << std::endl << std::endl;
    ...
    std::cout << "Doing a Beam Pattern scan using " << step << "° degree resolution steps !\n";
    ...
    for (int theta_tx = theta_min_tx; theta_tx <= theta_max_tx  ; theta_tx = theta_tx + step )
    {
        ...
        {
            std::cout << boost::format("MOVE Rx angle to %d ° at time %f") % theta_rx % Tx_mb_timeKper->get_time_now().get_real_secs() << std::endl;

            bool done_flag;
            std::cout << "Press when moving is done\n";
            std::cin >> done_flag;
            ...
            //Issue the Streaming at both tx/rx end !Nr_Of_Tx_beams
