...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    double 			tx_rate, rx_rate, freq_bb, gain_tx_bb, gain_rx_bb, lo_offset, bw, UpsampleFactor, Nr_Of_Rx_beams; 
    std::ofstream 	OutFile;
    std::ifstream   InputFile;
    uint64_t 		nbr_samps_per_degree;
    int             step_tx, step_rx, nbr_active_antennas, theta_min_tx, theta_max_tx, theta_min_rx, theta_max_rx;
    ...
    //std::cout << "first value to put inside tx angles is : " << theta_min_tx << std::endl;

    nbr_active_antennas = 1;

    std::string Exp_Name = "Moved_antenna_Setup/";

    std::string outputDirectory = Exp_Name + "Iterative_outputs_" + std::to_string(nbr_active_antennas) + "_antennas/";

    OutFilename = outputDirectory + "P1.dat";
	...
	//("step-tx", po::value<int>(&step_tx)->default_value((45 - (-45))/(Nr_Of_Tx_beams-1)), "Difference in degress between two succesive angles")
    //("step-rx", po::value<int>(&step_rx)->default_value((45 - (-45))/(Nr_Of_Rx_beams-1)), "Difference in degress between two succesive angles")
    ...
    //("OutFile", po::value<std::string>(&OutFilename)->default_value("Iterative_outputs_2_antennas/P1.dat"), "name of the file to read binary samples from")
    ("InputFile", po::value<std::string>(&InFilename)->default_value("SSB64_and_CSI.dat"), "name of the input file")   
    ...
    double Nr_Of_Tx_beams = 6;
    step_tx = 20;


    tx_angles = {-45, -25, -5, 5, 25, 45};

    double Rx_Beam_halfwidth;

    if (nbr_active_antennas == 4)
    {
        Rx_Beam_halfwidth = 20;
        Nr_Of_Rx_beams      = 6;
        rx_angles = {-45, -25, -5, 5, 25, 45};

        step_rx = Rx_Beam_halfwidth;
    }
    else if (nbr_active_antennas == 2)
    {
        Rx_Beam_halfwidth = 40;
        Nr_Of_Rx_beams      = 3;
        rx_angles = {-40, 0, 40};
        step_rx = Rx_Beam_halfwidth;
        theta_min_rx = -40;
    ...
    /*int curr_angle = theta_min_tx;
    //std::cout << "first value to put inside tx angles is : " << curr_angle << std::endl;
    //std::cout << "first value to put inside tx angles is : " << theta_min_tx << std::endl;

    for (int j = 0; j < Nr_Of_Tx_beams; j++)
    {
        tx_angles[j] = curr_angle;

        curr_angle += step_tx;
    }

    curr_angle = theta_min_rx;
    for (int j = 0; j < Nr_Of_Rx_beams; j++)
    {

        rx_angles[j] = curr_angle;
    ...
    // if there's still some sampling offset -> Comes from 
    ...
    int off_channels[4]; // Adjust size if necessary, based on maximum possible size
    int nbr_channels = 0;

    if (nbr_active_antennas == 1)
    {
        int temp[] = {2, 3, 4}; // Keeping only one channel
    ...
    { 
        int temp[] = {3, 4}; // Keeping the first 2 channels
        nbr_channels = sizeof(temp) / sizeof(temp[0]);
        memcpy(off_channels, temp, sizeof(temp));
    ...
    OutFile << boost::format("Gain tx is %d and Gain rx is %d") % Nr_Of_Tx_beams % Nr_Of_Rx_beams;
    ...
    OutFile << boost::format("Step is %d %d and N active antennas are %d") % step_tx % step_rx % nbr_active_antennas;
    ...
    std::vector<uint64_t> CSI_signal_address(Max_CSI_Stored);

    uint64_t Address_Offset = Max_SSBs_Stored * samps_per_packet * OTW_sample_size;

    for (int i = 0; i < Max_CSI_Stored; i++)
    {
        CSI_signal_address[i] = Address_Offset + (i * samps_per_packet * OTW_sample_size);
    }

    int CSI_beam_index = 0;
    ...
    //double appropriate_timeout = (nbr_samps_per_degree / rate) * overtime_Factor + issuing_time_frac + seconds_in_future;
    ...
    int theta_rx, theta_tx;

    char* Increasing_Rx_ptr = rx_buf_ptr;

    /*for (int theta_tx = theta_min_tx; theta_tx <= theta_max_tx  ; theta_tx = theta_tx + step_tx )
    {
        ...
        for(int theta_rx = theta_min_rx; theta_rx <= theta_max_rx  ; theta_rx = theta_rx + step_rx )
        {
    ...
    /* Now doing P2 Tx refinement process !*/

    LengthSignal = 566;

    double Selected_Tx_beam, Selected_Rx_beam;

    std::cout << "Best Tx Beam found at (deg°) ? : ";
    std::cin >> Selected_Tx_beam;

    std::cout << "Best Rx Beam found at (deg°) ? : ";
    std::cin >> Selected_Rx_beam;

    
    step_tx = 1;


    Nr_Of_Tx_beams = 10;// Number of refining beams around the selected Tx beam...
    Nr_Of_Rx_beams = 1; // Rx beam is now fixed during P2...

    double around_tx_view = step_tx * Nr_Of_Tx_beams;

    int steps_before_selected = std::floor((Nr_Of_Tx_beams-1) / 2);
    int steps_after_selected = (Nr_Of_Tx_beams-1) - steps_before_selected;

    std::cout << "Opening " << around_tx_view << "° around " << Selected_Tx_beam << "°, " << steps_before_selected*step_tx << "° before and " << steps_after_selected*step_tx << "° after.\n";
    

    std::ofstream OutFile_P2;

    OutFile_P2.open(outputDirectory + "P2.dat" , std::ofstream::binary);

    if (OutFile_P2.is_open()){
		printf("Output file opened correctly. \n"); }
    else
    {
		printf("OUTPUT FILE NOT OPENED !!! \n");
    ...
    //theta_rx = Selected_Rx_beam;
    theta_min_rx = Selected_Rx_beam;
    theta_max_rx = Selected_Rx_beam;

    theta_min_tx = Selected_Tx_beam - steps_before_selected*step_tx;
    theta_max_tx = Selected_Tx_beam + steps_after_selected*step_tx;

    std::cout << "About to sweep TX between " << theta_min_tx << "° and " << theta_max_tx << "°" << std::endl;

    step_rx = 0;

    if (OutFile_P2.is_open()) {
    OutFile_P2 << boost::format("Rate is %d and freq is %f") % tx_rate % freq_bb;
    OutFile_P2 << boost::format("Gain tx is %d and Gain rx is %d") % Nr_Of_Tx_beams % Nr_Of_Rx_beams;
    OutFile_P2 << boost::format("Number samps per degree is %d ") % nbr_samps_per_degree ;
    OutFile_P2 << boost::format("Thetas min and max of tx and rx are %d %d %d %d") % theta_min_tx % theta_max_tx % theta_min_rx %  theta_max_rx;
    OutFile_P2 << boost::format("Step is %d %d and N active antennas are %d") % step_tx % step_rx % nbr_active_antennas;
    OutFile_P2 << boost::format("Length of active packet is %d") % LengthSignal;
    OutFile_P2 << boost::format("Over Sampling at Receiver %d %d") % OverSampling % UpsampleFactor;
    }


    size_t TotalRxSamples_P2 = nbr_samps_per_degree* UpsampleFactor * (Nr_Of_Tx_beams*Nr_Of_Rx_beams);
    char* rx_buf_ptr_P2 = new char[TotalRxSamples_P2 * CPU_sample_size];

    SSB_beam_index = 0;

    char* Increasing_Rx_ptr_P2 = rx_buf_ptr_P2;
    ...
    /*for (int theta_tx = theta_min_tx; theta_tx <= theta_max_tx  ; theta_tx = theta_tx + step_tx )
    {
        ...
        size_t num_rx_samps = rx_stream->recv(Increasing_Rx_ptr_P2, nbr_samps_per_degree*UpsampleFactor, md, appropriate_timeout);
        ...
        Increasing_Rx_ptr_P2 = Increasing_Rx_ptr_P2 + CPU_sample_size*num_rx_samps;
    ...
    std::vector<int> tx_angles_P2 (Nr_Of_Tx_beams);
    std::vector<int> rx_angles_P2 (Nr_Of_Rx_beams);

    int curr_angle = theta_min_tx;
    int setting_angle;
    for (int j = 0; j < Nr_Of_Tx_beams; j++)
    {
        setting_angle = curr_angle;
    ...
    for (int j = 0; j < Nr_Of_Rx_beams; j++)
    {
        ...
        rx_angles_P2[j] = curr_angle;

        curr_angle += step_rx;
    }*/

    rx_angles_P2[0] = Selected_Rx_beam;


    std::cout << "Display the vectors containing the angles to loop on : \n";

    std::cout << "Tx vector angles : ";
    for (int k = 0; k < tx_angles_P2.size(); k++)
    {
        std::cout << tx_angles_P2[k] <<" ";
    }

    std::cout << "\nRx vector angles : ";
    for (int k = 0; k < rx_angles_P2.size(); k++)
    {
        std::cout << rx_angles_P2[k] <<" ";
    }
    std::cout << std::endl;


    for (int tx_ind = 0; tx_ind < tx_angles_P2.size(); tx_ind++)
    {
        //replay_ctrl->config_play(SSB_signal_address[tx_ind], packetSizeinBytes, replay_port);

        replay_ctrl->config_play(CSI_signal_address[tx_ind], packetSizeinBytes, replay_port);
        
        int currtheta_tx = tx_angles_P2[tx_ind];

        std::cout << boost::format("Setting Tx angle to %d ° at time %f") % currtheta_tx % Tx_mb_timeKper->get_time_now().get_real_secs() << std::endl;
        set_theta(service, sn_Tx_BBox, gain_dB_Tx, currtheta_tx);

        for(int rx_ind = 0; rx_ind < rx_angles_P2.size(); rx_ind++)
        {

            int currtheta_rx = rx_angles_P2[rx_ind];
    ...
    OutFile_P2.write((const char*)rx_buf_ptr_P2, TotalRxSamples_P2*CPU_sample_size*sizeof(char)); //Need to save = total_samples x (Bytes for each complex sampleCPU) x size of one char (=1Byte)
    ...
    /* Now doing P3 Rx refinement process !*/

    std::cout << "Best Refined (P2) Tx Beam found at (deg°) ? : ";
    std::cin >> Selected_Tx_beam;

    std::cout << "Now doing P3 Rx refinement, Fixed Tx Beam is : " << Selected_Tx_beam << "°, now refining RX beam around : " << Selected_Rx_beam << "°.\n";
    ...
    double around_rx_view = step_rx * Nr_Of_Rx_beams;

    steps_before_selected = std::floor((Nr_Of_Rx_beams-1) / 2);
    steps_after_selected = (Nr_Of_Rx_beams-1) - steps_before_selected;

    std::cout << "Opening " << around_rx_view << "° around " << Selected_Rx_beam << "°, " << steps_before_selected*step_rx << "° before and " << steps_after_selected*step_rx << "° after.\n"; 


    std::ofstream OutFile_P3;

    OutFile_P3.open(outputDirectory + "P3.dat" , std::ofstream::binary);

    if (OutFile_P3.is_open()){
		printf("Output file opened correctly. \n"); }
    else
    {
		printf("OUTPUT FILE NOT OPENED !!! \n");
    ...
    theta_tx = Selected_Tx_beam;
    theta_min_tx = Selected_Tx_beam;
    theta_max_tx = Selected_Tx_beam;



    theta_min_rx = Selected_Rx_beam - steps_before_selected*step_rx;
    theta_max_rx = Selected_Rx_beam + steps_after_selected*step_rx;

    step_tx = 0;

    nbr_active_antennas = 4;

    // If the set_theta() command doesn't set back all antennas by default -> need to rebuild the library...
    /*int temp[] = {1, 2, 3, 4}; // Turn back on all channels !
    nbr_channels = sizeof(temp) / sizeof(temp[0]);
    memcpy(off_channels, temp, sizeof(temp));
    ...
    if (OutFile_P3.is_open()) {
    OutFile_P3 << boost::format("Rate is %d and freq is %f") % tx_rate % freq_bb;
    OutFile_P3 << boost::format("Gain tx is %d and Gain rx is %d") % Nr_Of_Tx_beams % Nr_Of_Rx_beams;
    OutFile_P3 << boost::format("Number samps per degree is %d ") % nbr_samps_per_degree ;
    OutFile_P3 << boost::format("Thetas min and max of tx and rx are %d %d %d %d") % theta_min_tx % theta_max_tx % theta_min_rx %  theta_max_rx;
    OutFile_P3 << boost::format("Step is %d %d and N active antennas are %d") % step_tx % step_rx % nbr_active_antennas;
    OutFile_P3 << boost::format("Length of active packet is %d") % LengthSignal;
    OutFile_P3 << boost::format("Over Sampling at Receiver %d %d") % OverSampling % UpsampleFactor;
    }

    

    size_t TotalRxSamples_P3 = nbr_samps_per_degree* UpsampleFactor * (Nr_Of_Tx_beams*Nr_Of_Rx_beams);
    char* rx_buf_ptr_P3 = new char[TotalRxSamples_P3 * CPU_sample_size];

    SSB_beam_index = 0;

    char* Increasing_Rx_ptr_P3 = rx_buf_ptr_P3;
    ...
    for(int theta_rx = theta_min_rx; theta_rx <= theta_max_rx  ; theta_rx = theta_rx + step_rx )
    {
        ...
        size_t num_rx_samps = rx_stream->recv(Increasing_Rx_ptr_P3, nbr_samps_per_degree*UpsampleFactor, md, appropriate_timeout);
        ...
        Increasing_Rx_ptr_P3 = Increasing_Rx_ptr_P3 + CPU_sample_size*num_rx_samps;

        SSB_beam_index++;
    }*/
    

    std::vector<int> tx_angles_P3 (Nr_Of_Tx_beams);
    std::vector<int> rx_angles_P3 (Nr_Of_Rx_beams);

    /*curr_angle = theta_min_tx;
    for (int j = 0; j < Nr_Of_Tx_beams; j++)
    {
        if (curr_angle > 45)
        {
        ...
        tx_angles_P3[j] = curr_angle;

        curr_angle += step_tx;
    }*/

    tx_angles_P3[0] = Selected_Tx_beam;
    ...
    std::cout << "Tx vector angles : ";
    for (int k = 0; k < tx_angles_P3.size(); k++)
    {
        std::cout << tx_angles_P3[k] <<" ";
    }

    std::cout << "\nRx vector angles : ";
    for (int k = 0; k < rx_angles_P3.size(); k++)
    {
        std::cout << rx_angles_P3[k] <<" ";
    }
    std::cout << std::endl;



    for (int tx_ind = 0; tx_ind < tx_angles_P3.size(); tx_ind++)
    {
        //replay_ctrl->config_play(SSB_signal_address[tx_ind], packetSizeinBytes, replay_port);

        int currtheta_tx = tx_angles_P3[tx_ind];

        std::cout << boost::format("Setting Tx angle to %d ° at time %f") % currtheta_tx % Tx_mb_timeKper->get_time_now().get_real_secs() << std::endl;
        set_theta(service, sn_Tx_BBox, gain_dB_Tx, currtheta_tx);

        for(int rx_ind = 0; rx_ind < rx_angles_P3.size(); rx_ind++)
        {

            replay_ctrl->config_play(CSI_signal_address[rx_ind], packetSizeinBytes, replay_port);

            int currtheta_rx = rx_angles_P3[rx_ind];
    ...
    OutFile_P3.write((const char*)rx_buf_ptr_P3, TotalRxSamples_P3*CPU_sample_size*sizeof(char)); //Need to save = total_samples x (Bytes for each complex sampleCPU) x size of one char (=1Byte)
